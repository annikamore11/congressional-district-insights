...
export default function FECContent({ member, fecCache, setFecCache }) {  
  ...
  const destroyAllCharts = () => {
    try {
      const chartIds = ["financeChart", "contributionPie", "stateBarChart"];
      chartIds.forEach(id => {
  ...
  const drawPieChart = (agg) => {
    ...
    if (!pieCtx) {
      console.warn("contributionPie canvas not found");
      return;
    }

    try {
      const existingChart = Chart.getChart("contributionPie");
      if (existingChart) {
        existingChart.destroy();
      }
  ...
  const drawStateChart = (dataState) => {
    ...
    if (!stateCtx) {
      console.warn("stateBarChart canvas not found");
      return;
    }

    try {
      const existingChart = Chart.getChart("stateBarChart");
      if (existingChart) {
        existingChart.destroy();
      }
  ...
  return (
    <div className="p-2 md:p-4 bg-indigo-100 min-h-screen">
        ...
        {hasFecData && aggregatedData ? (
                  ...
                  <table className="min-w-full">
                    <thead className="bg-slate-50 border-b border-slate-200 sticky top-0">
                      <tr>
                        ...
                        <th className="text-left px-6 py-3 text-sm font-semibold text-slate-700">Contributor</th>
                        <th className="text-right px-6 py-3 text-sm font-semibold text-slate-700">Amount</th>
                      </tr>
                    </thead>
          ...
          <div className="bg-white rounded-lg shadow-md p-12 border border-slate-200 text-center">
            <div className="max-w-md mx-auto">
              ...
              <h3 className="text-lg font-semibold text-slate-800 mb-2">No FEC Data Available</h3>
              <p className="text-slate-600">Campaign finance information is not available for this representative.</p>
            </div>
          </div>
        )}
      </div>
